...
namespace SimulationRequest.Core
{
    ...
    public class HttpHelper
    {
        ...
        #endregion
        #region Test
        public bool CheckProxyIP(string proxy)
        {
            System.Net.ServicePointManager.DefaultConnectionLimit = 200;
            WebProxy proxyObject = new WebProxy(proxy);//str为IP地址 port为端口号  
            System.GC.Collect();
            HttpWebRequest req = (HttpWebRequest)WebRequest.Create("http://tool.oschina.net/");
        ...
        public string GetMayUseProxyIp(List<string> ips, string testUrl = "https://www.baidu.com", int timeout = 1000)
        {
            ...
            {
                result = http.GetHtml(httpItem, 512);
        ...
        public HttpResult UseProxyRequestV2(ProxyHelper proxyHelper, ref List<string> ips, string requestUrl, bool useOne = false, HttpItem httpItem = null, HttpHelper http = null)
        {
            if (ips.Count == 0)
            {
                ips = proxyHelper.GetIpsByApi();
            }
            if (http == null)
            {
                http = new HttpHelper();
            ...
            if (string.IsNullOrEmpty(httpItem.URL) && !string.IsNullOrEmpty(requestUrl))
            {
                httpItem.URL = requestUrl;
            }
            if (string.IsNullOrEmpty(httpItem.URL))
            {
                return null;
            }
            httpItem.ProxyIp = ips.FirstOrDefault();
            var result = http.GetHtml(httpItem, 512);
            if (useOne)
            {
                ips = ips.Skip(1).ToList();
            }
            if (result.StatusCode != System.Net.HttpStatusCode.OK)
            {
                ...
                return UseProxyRequestV2(proxyHelper, ref ips, requestUrl, useOne, httpItem, http);
            }
            return result;
        }
        public HttpResult UseProxyRequest(IEnumerable<string> ips, string requestUrl, HttpItem httpItem = null, HttpHelper http = null)
        {
            if (http == null)
            {
                http = new HttpHelper();
            ...
            if (result.StatusCode != System.Net.HttpStatusCode.OK)
            {
                return UseProxyRequest(ips.Skip(1), requestUrl, httpItem, http);
            }
            return result;
        }
        #endregion
        ...
        public HttpResult GetHtml(HttpItem item,int? defConnectionLimit=null)
        {
            if (defConnectionLimit.HasValue)
            {
                System.Net.ServicePointManager.DefaultConnectionLimit = defConnectionLimit.Value;
            }
            //返回参数
